...
public class StatisticsDialog extends JDialog {
	...
	public StatisticsDialog(WordleFrame view, WordleModel model, boolean vic) { //Victory variable to determine if correct word is shown; false for loss (show word), true for win (do not show word)
	...
	private JPanel selectPanel()
	{
		...
		{
			return this.createMainPanel_Win();
		}
	}
	...
	private JPanel createMainPanel_Win() {
		JPanel panel = new JPanel(new BorderLayout());
		panel.setBorder(BorderFactory.createEmptyBorder(0, 5, 5, 5));
		
		panel.add(createTopPanel_Win(), BorderLayout.NORTH);
		panel.add(createBottomPanel(), BorderLayout.SOUTH);
	...
	private JPanel createTopPanel() {
		...
		panel.add(correctWord(), BorderLayout.NORTH);
		panel.add(createTitlePanel(), BorderLayout.CENTER);
		panel.add(createSummaryPanel(), BorderLayout.SOUTH);
		
		return panel;
	}
	
	private JPanel createTopPanel_Win() {
		JPanel panel = new JPanel(new BorderLayout());
		panel.setBorder(BorderFactory.createEmptyBorder(0, 5, 5, 5));
	...
	private JPanel correctWord()
	{
		JPanel panel = new JPanel(new FlowLayout());
		panel.setBorder(BorderFactory.createEmptyBorder(0, 5, 5, 5));
		
		JLabel label = new JLabel("The correct word was " + this.model.getCurrent() + ".");
