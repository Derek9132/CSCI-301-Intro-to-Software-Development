...
public class KeyboardButtonAction extends AbstractAction {
	...
	@Override
	public void actionPerformed(ActionEvent event) { //Handles input
		...
		case "Enter":
			if (model.getCurrentColumn() >= (model.getColumnCount() - 1)) {
				...
				if (greenCount >= model.getColumnCount()) { //word is correct, end current game
					...
					new StatisticsDialog(view, model, true); //Makes new file
					//System.out.println("file made");
				} else if (!moreRows) { //Word is incorrect
					view.repaintWordleGridPanel();
					model.getStatistics().incrementTotalGamesPlayed();
					model.getStatistics().setCurrentStreak(0);
					new StatisticsDialog(view, model, false); //Makes new file
					//System.out.println("file made");
				} else {
					view.repaintWordleGridPanel();
				}
